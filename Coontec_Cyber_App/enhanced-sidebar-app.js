...
const menuStructure = [
  {
    ...
    href: '2-1_메인대시보드.html'
  },
  {
    id: 'notice',
    name: '공지사항',
    icon: 'megaphone',
    href: '2-2_프로젝트현황.html'
  },
  {
    id: 'projects',
    name: '프로젝트 관리',
    icon: 'folder-open',
    href: '4-3_프로젝트상세.html'
  },
  {
    id: 'tests',
    name: '시험 관리',
    icon: 'file-search',
    submenu: [
      { name: '스캔시험', href: '5-3_시험실행.html' },
      { name: '부하시험', href: '6-3_부하시험실행.html' }
    ]
  },
  {
    id: 'licenses',
    ...
    icon: 'credit-card',
    submenu: [
      { name: '라이선스 관리', href: '7-8_라이선스관리.html' }
    ]
  },
  {
    id: 'data',
  ...
  },
  {
    ...
    icon: 'users',
    submenu: [
      { name: '사용자 관리', href: '10-10_사용자관리.html' }
    ]
  },
  {
    id: 'settings',
  ...
  },
  {
    ...
    icon: 'shield',
    submenu: [
      { name: '감사로그', href: '14-1_사용자가이드.html' }
    ]
  },
  {
    id: 'notifications',
...
function generateProjectQuickAccess() {
  ...
  return `
    <div class="py-3 border-b border-gray-700">
      <div class="px-3 mb-3">
        <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-wider">프로젝트 바로가기</h3>
      </div>
      ...
      <div class="px-3 mb-3">
        <div class="relative">
          ...
          <input 
            ...
            class="w-full pl-7 pr-2 py-1 text-xs bg-gray-800 border border-gray-600 text-white rounded focus:outline-none focus:ring-1 focus:ring-purple-500"
            id="project-search"
          >
        </div>
      </div>
      ...
      <!-- Recent Projects -->
      <div class="space-y-2 px-3" id="recent-projects">
        ${recentProjects.map(project => `
          <div class="project-item">
            <button class="w-full text-left p-2 rounded hover:bg-gray-700 bg-gray-800 border border-gray-600" data-project="${project.id}">
              <div class="flex items-center justify-between">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron h-3 w-3 text-gray-500 transition-transform project-chevron">
                  ${iconSVGs['chevron-right']}
                </svg>
              </div>
              
              <!-- Ships -->
              <div class="project-ships ml-5 mt-2 space-y-1" style="display: none;">
                  ...
                  <div class="ship-item">
                    ...
                    <div class="ml-3 space-y-1">
                      ${ship.zones.map(zone => {
                        ...
                        return `
                          <div class="zone-item p-1 rounded bg-gray-700">
                            <div class="text-xs text-gray-300">${zone.name}</div>
      ...
      <div class="px-3 mt-3">
        <a href="4-3_프로젝트상세.html" class="text-xs text-purple-400 hover:text-purple-300 font-medium">전체 프로젝트 보기 →</a>
      </div>
    </div>
  `;
}
...
function generateRegularMenu(currentPage = '') {
  let html = '<nav class="flex-1 py-3 overflow-y-auto">';
  ...
  menuStructure.forEach(menu => {
    ...
    if (menu.submenu) {
      html += `
        <div class="menu-group px-3 mb-1">
          <button class="w-full flex items-center justify-between px-2 py-2 text-sm font-medium rounded hover:bg-gray-700 ${isActive ? 'bg-gray-700 text-purple-400' : 'text-gray-300'}" data-menu="${menu.id}">
            <div class="flex items-center gap-2">
          ...
          <div class="submenu mt-1 space-y-0.5" style="display: ${isActive ? 'block' : 'none'};">
            ${menu.submenu.map(sub => `
              <a href="${sub.href}" class="block ml-8 px-2 py-1.5 text-xs ${currentPage === sub.href ? 'text-purple-400 font-medium' : 'text-gray-400'} hover:text-purple-400 hover:bg-gray-700 rounded">
                ${sub.name}
              </a>
            `).join('')}
          </div>
      ...
      html += `
        <a class="flex items-center gap-2 mx-3 px-2 py-2 mb-1 text-sm font-medium rounded hover:bg-gray-700 ${isActive ? 'bg-gray-700 text-purple-400' : 'text-gray-300'}" href="${menu.href}">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-${menu.icon} h-4 w-4">
            ${iconSVGs[menu.icon]}
          </svg>
          <span>${menu.name}</span>
...
function generateEnhancedSidebar(currentPage = '') {
  return `
    ...
    <div class="border-t border-gray-700 px-3 py-3">
      <a class="flex items-center gap-2 px-2 py-2 text-xs text-gray-400 hover:text-white hover:bg-gray-700 rounded" href="2-2_프로젝트현황.html">
